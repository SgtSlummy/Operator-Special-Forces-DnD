...
test('campaign feed adapter renders the DM projection for a DM scope', async () => {
  ...
  assert.match(calls[0][2].data.embeds[0].fields[1].value, /hidden passage/); assert.equal(calls[0][2].data.flags & 64, 64); assert.equal(calls[0][2].data.embeds[0].title, 'DM master thread');
});
