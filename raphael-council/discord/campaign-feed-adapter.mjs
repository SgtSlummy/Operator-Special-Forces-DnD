import { AUDIENCES, recordRoll, ruleCheck, shareFact, submitIntent } from './campaign-feed-contract.mjs';
...
export function createCampaignFeedAdapter({ readFeed, writeFeed, authorize, transport, render = renderCampaignFeed, roll = ({ count, sides }) => Array.from({ length: count }, () => Math.floor(Math.random() * sides) + 1) } = {}) {
  ...
  return async interaction => {
    ...
    const ruleRequest = /^campaign:rule:([a-zA-Z0-9_-]{1,64}):(\d+)$/.exec(interaction?.data?.custom_id ?? '');
    if (ruleRequest) {
      const scope = await authorize(interaction, { action: 'rule', campaignId: ruleRequest[1] });
